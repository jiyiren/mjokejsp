...
public class GetUserAboutJokes extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public GetUserAboutJokes() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		PrintWriter out = response.getWriter();
		String result=null;
		String jokeid=request.getParameter(JokeConfig.JOKE_ID);
		String token=request.getParameter(JokeConfig.USERTOKEN);
		String count=request.getParameter(JokeConfig.COUNT);
		String abouttype=request.getParameter(JokeConfig.USER_ABOUT_JOKE_TYPE);//类型
		if(jokeid!=null&&token!=null&&count!=null&&abouttype!=null){
			int mcount=Integer.parseInt(count);
			if(mcount<=0){
				mcount=5;
			}
			List<SingleJoke> mjokes=new ArrayList<SingleJoke>();
			mjokes=new UserDao().getAboutJokes(jokeid, token, mcount,abouttype);
			if(mjokes==null){
