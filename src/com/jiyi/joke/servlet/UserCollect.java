...
public class UserCollect extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(joke_id!=null&&user_id!=null&&type!=null){
			UserCollectBean ucb=new UserCollectBean();
			ucb.setJoke_id(joke_id);
			ucb.setUser_id(user_id);
			ucb.setType(type);
			String re=new UserDao().DoUserCollect(ucb);
			if(re.equals("0")){
				result=OutJson.outUserCollect(OutJson.FAIL);
			}else if(re.equals("1")){
				result=OutJson.outUserCollect(OutJson.SUCCESS);
			}else{
				result=OutJson.outUserCollect(OutJson.FAIL_EXIST);
			}
		}else{
			result=OutJson.outUserCollect(OutJson.FAIL);
		}
